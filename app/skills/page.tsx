...
export default function Skills() {
  ...
  const totalCoreTech = Object.values(skillsData).reduce((acc, cat) => acc + cat.technologies.length, 0)
  const totalAdditionalTech = additionalSkills.categories.reduce((acc, cat) => acc + cat.technologies.length, 0)
  const totalTech = totalCoreTech + totalAdditionalTech
  const categoriesCount = Object.keys(skillsData).length + additionalSkills.categories.length
        ...
        <div className="p-6 md:p-8 flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 w-full lg:w-auto mt-4 lg:mt-0">
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-center">
              ...
              <p className="mt-1 text-2xl font-bold text-slate-900">{categoriesCount}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-center">
              <p className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-slate-500">
                <Layers className="h-3.5 w-3.5" />
                {language === "en" ? "Techs" : "Tecnologías"}
              </p>
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-center col-span-1">
              ...
              <p className="mt-1 text-2xl font-bold text-slate-900">3+ {language === "en" ? "Yrs" : "Años"}</p>
            </div>
          </div>
        </div>
      </div>
